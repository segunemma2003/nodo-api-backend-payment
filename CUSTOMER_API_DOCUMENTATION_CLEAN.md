...
**Request Body:**
- `business_name` (required, string, max 255): Business name
- `email` (required, email, unique): Email address
- `username` (required, string, unique): Username
- `password` (required, string, min 8 characters): Password
- `phone` (optional, string): Phone number
- `address` (optional, string): Physical address
- `minimum_purchase_amount` (optional, numeric, min 0): Minimum purchase amount
- `payment_plan_duration` (optional, integer, min 1, max 36): Payment plan duration in months
- `kyc_documents` (optional, array of files): KYC documents (pdf, jpg, jpeg, png, max 10MB each)
...
**⚠️ REQUIREMENT:** Account must be approved by admin before login is allowed.
...
- `in_grace`: Within grace period (no interest)
...
**URL Parameters:**
- `invoiceId`: Invoice ID (e.g., "FSCREDIT-2024-001")
...
6. **Interest Calculation:** 
   ...
   - After grace period: 3.5% per month for each month overdue
7. **Payment Plan Duration:** Used to calculate due dates and interest. Due date = purchase_date + payment_plan_duration months
8. **Grace Period:** 1 month after due date before additional interest applies
9. **Virtual Account:** 
...
### Invoice Object
- `invoice_id`: Invoice identifier (e.g., "FSCREDIT-2024-001")
- `purchase_date`: Date of purchase
- `due_date`: Payment due date (purchase_date + payment_plan_duration months)
- `grace_period_end_date`: End of 1-month grace period (due_date + 1 month)
- `payment_plan_duration`: Duration in months for payment plan
